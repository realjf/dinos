...
void* _darray_create(u64 length, u64 stride) {
    ...
    u64* new_array = dallocate(header_size + array_size, MEMORY_TAG_DARRAY);
...
void* _darray_resize(void* array) {
    ...
    void* temp = _darray_create(
        ...
        stride);
    dcopy_memory(temp, array, length * stride);
...
void* _darray_pop_at(void* array, u64 index, void* dest) {
    ...
    u64 stride = darray_stride(array);
    if (index >= length) {
        DERROR("Index outside the bounds of this array! Length: %i, index: %index", length, index);
        return array;
    }

    u64 addr = (u64)array;
    dcopy_memory(dest, (void*)(addr + (index * stride)), stride);
...
void* _darray_insert_at(void* array, u64 index, void* value_ptr) {
    ...
    dcopy_memory((void*)(addr + (index * stride)), value_ptr, stride);
